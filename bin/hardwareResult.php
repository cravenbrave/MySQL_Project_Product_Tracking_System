 ...
 if(!empty($_POST["mf"]) && !empty($_POST["model"])){
   ...
   $model = $_POST["model"];
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      header('Location: http://localhost:8080/project/src/errorHardware.php');
    }
...
if(!empty($_POST["mf"]) && !empty($_POST["model"]) && !empty($_POST["OS"]) && !empty($_POST["hardwarePurDate"]) && !empty($_POST["OS"])){
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    //if there is a max sysNo and no same tuple exists
    if (mysqli_num_rows($result2) > 0 && mysqli_num_rows($result3) == 0 ) {
      ...
      //if there is a max sysNo. but OS is already in the table, show error:
  ...
  //if there is any reqiured item empty, head back to hardware.php 
  header("Location: http://localhost:8080/project/src/hardware.php");
}
