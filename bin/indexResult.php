...
//if user enter cusName, contactName and contactNo
if(!empty($_POST["cusName"]) && !empty($_POST["contactName"]) && !empty($_POST["contactNo"])){
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while($row = mysqli_fetch_assoc($result)) {
        ...
        echo "Contact Name is " . $row["cusName"]." <br>";
...
$conn->close();
if(empty($_POST["cusName"]) || empty($_POST["contactName"]) || empty($_POST["contactNo"])){
  header("Location: http://localhost:8080/project/src/index.php");
}
?>
